...
public:
   ...
   {       
      //list.delQueue();   // îïóñòàøàåì ñïèñîê ïåðåä çàïîëíåíèåì
      resetList();
      if (method == 0)
         in_order(Root);   // Ñèììåòðè÷íûé èëè ïîïåðå÷íûé - îáðàáàòûâàåòñÿ ëåâàÿ âåòêà, êîðåíü, ïðàâàÿ âåòêà
      if (method == 1)
         post_order(Root); // îáðàòíûé îáõîä - ãîît îáðàáàòûâàåòñÿ â êîíöå
      if (method == 2)
         pre_order(Root);  // ïðÿìîé îáõîä - ãîît îáðàáàòûâàåòñÿ ïåðâûì
      //return &list;
      return List;
   }
   ...
   BSTNode** WideAllNodes()
   {
       //list.delQueue();     // îïóñòàøàåì ñïèñîê ïåðåä çàïîëíåíèåì 
       resetList();
       Queue *q = new Queue();        //Äëÿ íà÷àëà ïîìåñòèì â î÷åðåäü êîðåíü    
       q->addToQueue(Root); 
       int i = 0;
       while ( !q->isEmpty() ) {
           ...
           List[countArr++] = tmp;
           //list.addToQueue(tmp);        
           if (tmp->LeftChild) {                 //Åñëè åñòü ëåâûé íàñëåäíèê, òî ïîìåùàåì åãî â î÷åðåäü äëÿ äàëüíåéøåé îáðàáîòêè
               q->addToQueue(tmp->LeftChild);
           }         
           if (tmp->RightChild) {                //Åñëè åñòü ïðàâûé íàñëåäíèê, òî ïîìåùàåì åãî â î÷åðåäü äëÿ äàëüíåéøåé îáðàáîòêè
